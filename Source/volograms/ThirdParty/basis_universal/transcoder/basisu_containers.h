...
#define BASISU_HAS_DESTRUCTOR(T) ((!scalar_type<T>::cFlag) && (!__is_pod(T)))
   ...
   public:
      ...
      bool try_reserve(size_t new_capacity_size_t)
      {
         if (new_capacity_size_t > UINT32_MAX)
         {
            assert(0);
      ...
      void resize(size_t new_size_size_t, bool grow_hint = false)
      {
          //TODO(Jan): Fix this strange compilation error
      ...
      bool try_resize(size_t new_size_size_t, bool grow_hint = false)
      {
         if (new_size_size_t > UINT32_MAX)
         {
            assert(0);
